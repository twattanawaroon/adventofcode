...
while dead_elf:
    ...
    grid.clear()
    for line in inp_grid:
        grid.append([getObj(c) for c in line])

    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if isinstance(grid[i][j], Unit):
                grid[i][j].setXY(i, j)
            if isinstance(grid[i][j], Elf):
                grid[i][j].setAtk(elf_power)
        ...
        for unit in units:
            ...
            if bestAdjEnemy is not None:
                ...
                if bestAdjEnemy.hp <= 0:
                    ...
                    if isinstance(bestAdjEnemy, Elf):
                        dead_elf = True
                        done = True
                        break
        if done:
